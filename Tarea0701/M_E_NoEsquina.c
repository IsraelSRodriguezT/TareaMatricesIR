    ...
    for(int i=0;i<filas;i++){ //Bucle for para indicarle a la matriz en que posicion de fila debe de estar
        ...
        for(int j=0;j<columnas;j++){ //Bucle for para indicarle a la matriz en que posicion de columna debe de estar
            if((i==0&&j==0)||(i==0&&j==(columnas-1))||(i==(filas-1)&&j==0)||(i==(filas-1)&&j==(columnas-1))){
                printf("   |"); //Para el caso de que si se cumple la condicion, que se imprima un espacio vacio en la matriz
            } else{
                printf(" %d |", matriz[i][j]); //Para el caso de que no se cumpla la condicion, que se imprima el valor de la matriz
            } 
        }
        printf("\n"); //Para hacer un salto de linea al momento de cambiar de fila
    }
...
int main(){
    printf("Bienvenido al programa para no imprimir los 4 elementos de la esquina de una matriz (la matriz debe de ser cuadrada)\n");
